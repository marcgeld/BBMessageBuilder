...
static NSString* const	kMessageBuilderBBLMStringTag = @"se.redseven.bblm.MessageBuilder.stringTag";
static NSString* const	kMessageBuilderBBLMStringTagAttr = @"se.redseven.bblm.MessageBuilder.stringTagAttr";
static NSString* const	kMessageBuilderBBLMStringTagVal = @"se.redseven.bblm.MessageBuilder.stringTagVal";
...
static bool colorstr( UniChar delim, BBLMParamBlock &pb, struct runloc &r, BBLMTextIterator &p,
    ...
    while (c && more_runs) {
        ...
        // Color python string subst things
            ...
            if ( isalpha(c) ) {
                ...
                more_runs = addRunAt(tag?kMessageBuilderBBLMStringTag:kBBLMSingleQuotedStringRunKind,r,bblm_callbacks,-2)
                && addRunAt(kMessageBuilderBBLMStringSubst,r,bblm_callbacks);
            }
            else if (c == '(') {
            ...
            if ( chars[1] ==')' && isalpha(c)) {
                ...
                more_runs = addRunAt(kMessageBuilderBBLMStringSubst,r,bblm_callbacks);
                lookup = false;
            }
        }
        ...
        // html in strings
            ...
            more_runs = addRunAt(kMessageBuilderBBLMStringProcDir,r,bblm_callbacks,0);
            xmlprocdir = false;
        }
            ...
            if (--htmlcomment == 0) {
                ...
                more_runs = addRunAt(kMessageBuilderBBLMStringComment,r,bblm_callbacks,0);
            }
        }
        else if (tag) {
            ...
            if ( chars[1] == '=' && c == '"' ) {
                ...
                more_runs = addRunAt(kMessageBuilderBBLMStringTagAttr,r,bblm_callbacks,-1);
                qtagval = true;
            }
                ...
                if ( wtagval ) {
                    ...
                    more_runs = addRunAt(kMessageBuilderBBLMStringTagVal,r,bblm_callbacks,-1);
                    wtagval = false;
                }
                more_runs = addRunAt(kMessageBuilderBBLMStringTag,r,bblm_callbacks);
                tag = false;
            }
            else if (wtagval && isspace(c)){
                
                more_runs = addRunAt(kMessageBuilderBBLMStringTagVal,r,bblm_callbacks);
                wtagval = false;
            }
            else if (qtagval && c=='"'){
...
static bool isStringKind( NSString *kind )
{
                  ...
                  ^()
                  {
                      ...
                      [[NSSet setWithArray:
                        @[
                          ...
                          kMessageBuilderBBLMStringSubst,
                          kMessageBuilderBBLMStringTag,
                          kMessageBuilderBBLMStringTagAttr,
                          kMessageBuilderBBLMStringTagVal,
                          kMessageBuilderBBLMStringProcDir,
                          kMessageBuilderBBLMStringComment,
                          ]
                        ] retain];
                  }
                  );
...
static void eat3str(char delim,BBLMParamBlock &pb, UInt32& pos)
{
    BBLMTextIterator asciText(pb);
    
    while (pos<pb.fTextLength){
        
        if (asciText[pos] == asciText[pos-1] == asciText[pos-2] == delim
                && (asciText[pos-3] != '\\' || asciText[pos-4] == '\\')) {
...
static void eat_line(BBLMParamBlock &pb, UInt32& pos)
{
    BBLMTextIterator asciText(pb);
...
static void addItem( BBLMProcInfo *procInfo, UInt32 pos, int nest, BBLMFunctionKinds kind,
                    const BBLMParamBlock &pb, const BBLMCallbackBlock *bblm_callbacks )
{
    ...
    while ((isalnum(asciText[pos])||asciText[pos]=='_') && pos<pb.fTextLength) {
        pos++; funcnamelen++;
    }
...
static void commitfunc(BBLMProcInfo &proc, unsigned int funcEnd,
                       BBLMParamBlock &pb, const BBLMCallbackBlock *bblm_callbacks)
{
...
//static bool	text_has_shebang(BBLMParamBlock &params)
//{
//    
//    static dispatch_once_t	_inited;
...
/*
static OSErr GuessLanguage(BBLMParamBlock &params)
{
...
extern "C"
{
    ...
    OSErr MessageBuilderMachO(BBLMParamBlock &params, const BBLMCallbackBlock &bblm_callbacks);
